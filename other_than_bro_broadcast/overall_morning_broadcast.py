...
task_df = task_df[
    (task_df["status"] == "Pending") | (task_df["status"] == "Re-Schedule")
]
...
collection_task_df = (
    collection_task_df.groupby("assigned_to")["deadline_date"].count().reset_index()
)
collection_task_df = collection_task_df.rename(
    columns={"deadline_date": "collection_count"}
)

proposal_task_df = task_df[task_df["category"] == "5"]
proposal_task_df = (
    proposal_task_df.groupby("assigned_to")["deadline_date"].count().reset_index()
)
...
retention_task_df = (
    retention_task_df.groupby("assigned_to")["deadline_date"].count().reset_index()
)
retention_task_df = retention_task_df.rename(
    columns={"deadline_date": "retention_count"}
)

wrong_number_task_df = task_df[task_df["category"] == "10"]
wrong_number_task_df = (
    wrong_number_task_df.groupby("assigned_to")["deadline_date"].count().reset_index()
)
wrong_number_task_df = wrong_number_task_df.rename(
    columns={"deadline_date": "wrong_number_count"}
)

task_df = task_df[(task_df["status"] == "Completed")]
completed_task_df = (
    task_df.groupby("assigned_to")["deadline_date"].count().reset_index()
)
completed_task_df = completed_task_df.rename(
    columns={"deadline_date": "Completed_Count"}
)

main_df = pd.merge(
    main_df, pending_task_df, left_on="UserID", right_on="assigned_to", how="right"
)
main_df = pd.merge(
    main_df, collection_task_df, left_on="UserID", right_on="assigned_to", how="left"
)
main_df = pd.merge(
    main_df, proposal_task_df, left_on="UserID", right_on="assigned_to", how="left"
)
main_df = pd.merge(
    main_df, retention_task_df, left_on="UserID", right_on="assigned_to", how="left"
)
main_df = pd.merge(
    main_df, wrong_number_task_df, left_on="UserID", right_on="assigned_to", how="left"
)


main_df = (
    main_df.drop(columns=["assigned_to_y", "assigned_to_x"], axis=1)
...
main_df["wrong_number_count"] = main_df["wrong_number_count"].astype(
    int, errors="ignore"
)
...
for data in all_df.to_dict("r"):
    """Updating lead list"""
    field_5_string = f"।।आपके पास।।।।,"  ## stores all counts as formatted string

    pending_count = data["Pending_Count"]
    field_5_string += (
        (str(pending_count) + pending_count_hindi_text + "। जिनमें से ,")
        if pending_count != 0
        else ""
    )
    ...
    field_5_string += (
        (str(collection_count) + collection_count_hindi_text)
    ...
    field_5_string += (
        (str(proposal_count) + proposal_count_hindi_text) if proposal_count != 0 else ""
    )

    retention_count = data["retention_count"]
    field_5_string += (
        (str(retention_count) + retention_count_hindi_text)
    ...
    field_5_string += (
        (str(wrong_number_count) + wrong_number_count_hindi_text)
